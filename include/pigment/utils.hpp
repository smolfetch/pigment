    ...
    namespace utils {
        ...
        struct ColorBlindness {
            ...
            static RGB simulate(const RGB& color, Type type) {
                ...
                return RGB(
                    std::clamp(static_cast<int>(nr * 255), 0, 255),
                    std::clamp(static_cast<int>(ng * 255), 0, 255),
                    std::clamp(static_cast<int>(nb * 255), 0, 255),
                    color.a
                );
            }
        };
        ...
        AccessibilityLevel::Level check_accessibility(const RGB& foreground, const RGB& background, bool large_text = false) {
            double ratio = contrast_ratio(foreground, background);
            
            if (ratio >= 7.0) return AccessibilityLevel::AAA_NORMAL;
            if (ratio >= 4.5) {
                return large_text ? AccessibilityLevel::AAA_LARGE : AccessibilityLevel::AA_NORMAL;
            }
            if (ratio >= 3.0 && large_text) return AccessibilityLevel::AA_LARGE;
            
            return AccessibilityLevel::FAIL;
        }

        // Find the best contrasting color (black or white)
        RGB best_contrast_color(const RGB& background) {
        ...
        double color_temperature(const RGB& color) {
        ...
        bool is_warm_color(const RGB& color) {
            return color_temperature(color) < 5000; // Below daylight temperature
        }

        // Generate a harmonious color scheme
        std::vector<RGB> generate_harmony(const RGB& base, const std::string& scheme = "complementary") {
        ...
        void sort_by_hue(std::vector<RGB>& colors) {
        ...
        void sort_by_brightness(std::vector<RGB>& colors) {
            std::sort(colors.begin(), colors.end(), [](const RGB& a, const RGB& b) {
                return a.luminance() < b.luminance();
            });
        }

        void sort_by_saturation(std::vector<RGB>& colors) {
            std::sort(colors.begin(), colors.end(), [](const RGB& a, const RGB& b) {
                HSL hsl_a = HSL::fromRGB(a);
                HSL hsl_b = HSL::fromRGB(b);
        ...
        double color_distance(const RGB& color1, const RGB& color2) {
        ...
        RGB find_closest_color(const RGB& target, const std::vector<RGB>& palette) {
            if (palette.empty()) return target;
        ...
        std::vector<RGB> quantize_to_palette(const std::vector<RGB>& colors, const std::vector<RGB>& palette) {
